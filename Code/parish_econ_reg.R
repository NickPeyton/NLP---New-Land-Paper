...
pdf = read.csv(glue('{PROCESSED}/parish_dataset.csv'), check.names=FALSE)
...
pdf$monast_land_share <- log(pdf$landOwned + 1)

# Divide monast_land_share by area
pdf$monast_land_share_area <- pdf$monast_land_share / pdf$area

# Monastic land share calculation
pdf$monast_land_share <- (pdf$landOwned / 240) / (pdf$parish_val_1524 + pdf$landOwned / 240)
...
weightitmodel <- weightit(monast_land_share ~ mean_slope + wheatsuit + 
                            area + lspc1525 + distmkt + parish_master_treatment_1524 +
                            parish_master_control_1524,
                          data=rdf,
                          method='ps'
)
...
stargazer(ind_weighted_rlm, agr_weighted_rlm, other_weighted_rlm,
          ...
          omit = 'Constant',
          out = 'Output/Tables/IPW_parish.tex'
)
###############################################################################
###############################################################################

rdf<-pdf
rdf <- rdf %>% filter(!is.na(parish_master_treatment_1524))
weightitmodel <- weightit(parish_master_treatment_1524 ~ mean_slope + wheatsuit + 
...
weights <- weightitmodel$weights
weighted_rlm <- rlm(agr_1831 ~ parish_master_treatment_1524,
                  weights=weights,
                  data=rdf)
...
weights <- weightitmodel$weights
weighted_rlm <- rlm(ind_1831 ~ parish_master_treatment_1524,
                  weights=weights,
                  data=rdf)
...
weights <- weightitmodel$weights
weighted_rlm <- rlm(other_1831 ~ parish_master_treatment_1524,
                  weights=weights,
                  data=rdf)
...
rdf<-pdf
rdf <- rdf %>% filter(!is.na(parish_master_control_1524))
weightitmodel <- weightit(parish_master_control_1524 ~ mean_slope + wheatsuit + 
                            area + lspc1525 + distmkt + monast_land_share + 
                            parish_master_treatment_1524,
                          data=rdf,
                          method='ps'
)
weights <- weightitmodel$weights
weighted_rlm <- rlm(agr_1831 ~ parish_master_control_1524,
                    weights=weights,
                    data=rdf)
...
weights <- weightitmodel$weights
weighted_rlm <- rlm(ind_1831 ~ parish_master_control_1524,
                    weights=weights,
                    data=rdf)
...
weights <- weightitmodel$weights
weighted_rlm <- rlm(other_1831 ~ parish_master_control_1524,
                    weights=weights,
                    data=rdf)
