...
# %% Load data
df = pd.read_csv(f'{PROCESSED}/parish_data.csv', encoding='utf-8')
...
df['monast_land_share'] = (df['landOwned'] / 240) / (df['tot_val_1524'] + df['landOwned'] / 240)
...
for year in [1524, 1543, 1581, 1674, 1840]:
    for measure in ['count', 'tot_val']:
        ...
        avg_val_reg = summary_col(result_list, stars=True,
                                  float_format='%0.2f',
                                  model_names=['1524-1581', '1581-1674', '1674-1840', '1524-1840'],
        ...
        for table, filename in zip([avg_val_reg, total_wealth_reg, max_val_reg], ['avg_val_reg', 'total_wealth_reg', 'max_val_reg']):
            table = table.replace(
                ...
                '\\end{center}\n* p<.1, ** p<.05, ***p<.01\n\\end{table}')
            table = table.replace('<', '$<$').replace('>', '$>$').replace('\\label{}',
                                                                                    '\\label{parish_' + measure + '_' + str(
                                                                                        year) + '}')
            ...
            with open(f'{TABLES}/{filename}_parish_{measure}_{year}.tex', 'w', encoding='utf-8') as f:
                f.write(table)
            print(table)
        ...
        ag_ind_reg = ag_ind_reg.replace('<', '$<$').replace('>', '$>$').replace('\\label{}', '\\label{parish_' + measure + '_' + str(year) + '}')
        ...
        print(ag_ind_reg)
        with open(f'{TABLES}/ag_ind_reg_{measure}_{year}.tex', 'w', encoding='utf-8') as f:
            f.write(ag_ind_reg)
